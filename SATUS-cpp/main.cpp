...
void generateNewPhotons(int current_iter, double x20, int Nphotons, double dr, Mesh &simmesh, std::vector< std::vector< Photon > > &photonArray, std::mt19937 gen, std::normal_distribution<double> x10_dist) {
    ...
    for (int i=0; i<Nphotons; ++i) {
        ...
        photonArray[current_iter][i].resetPhotonParams(x10, x20, ind0.indx1, ind0.indx2, phi0, dr);
    }
...
int main() {
    ...
    std::vector<std::vector< Photon > > photonArray(pulselength, std::vector<Photon>(Nphotons, Photon(0.0, 0.0, 0, 0, M_PI / 2.0, dr)));
    ...
    for (int iter=0; iter<=maxIter; ++iter) {
        ...
        if (iter < pulselength && iter > 0) {
            generateNewPhotons(iter, x2min, Nphotons, dr, simmesh, photonArray, gen, x10_dist);
        }
        
        simmesh.SnapToMesh(iter, photonArray);
        
        // Looping through each photon, 
        // probably could slap a pramga on here with OMP
        for (int level=0; level<iter; ++level) {
            
            if (level < photonArray.size()) {

                for (int p=0; p<photonArray[level].size(); p++) {
                    bool is_photon_alive = photonArray[level][p].isAlive();

                    if (is_photon_alive == true) {
                        ind phot_ind = photonArray[level][p].getIndexPos();

                        double p_abs = photonArray[level][p].probAbsorb(simmesh.mesh[phot_ind.indx2][phot_ind.indx1].abs_coeff);

                        if (prob(gen) < p_abs) {
                            photonArray[level][p].setAlive(false);
                        } else {

                            double p_scat = photonArray[level][p].probScatter(simmesh.mesh[phot_ind.indx2][phot_ind.indx1].sct_coeff);
                            ...
                            photonArray[level][p].getNewDir(scatter, simmesh.mesh[phot_ind.indx2][phot_ind.indx1].aniso_coeff);
                            photonArray[level][p].Move();
